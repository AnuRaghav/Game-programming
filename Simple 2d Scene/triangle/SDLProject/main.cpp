...
constexpr float DELTA_TIME = 0.166f;
...
void initialise()
{
    ...
    g_triangle= glm::vec3(g_model_x, g_model_x,0.0f);
...
void update() {
    ...
    g_model_x += 0.5f * delta_time;
    g_model_y += 0.5f * delta_time;
    g_triangle.x = cos(g_model_x) ;
    g_triangle.y = sin(g_model_y);
    
    g_model_matrix = glm::mat4(1.0f);
    
//    g_triangle = glm::vec3(glm::cos(g_model_x), glm::sin(g_model_y), 0.0f);
    
    g_model_matrix = glm::translate(g_model_matrix, g_triangle);  // MOVES LINEARLY SIDEWAYS IN DELTATIME
