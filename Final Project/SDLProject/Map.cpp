...
Map::Map(int width, int height, int *levelData, int *furnitureData, GLuint textureID, float tile_size, int tile_count_x, int tile_count_y) {
...
void Map::Build() {
    for(int y = 0; y < this->height; y++) {
        for(int x = 0; x < this->width; x++) {
            ...
            if (tile < -1 || tile >= tile_count_x * tile_count_y) continue;
            if (tile == -1) continue;
    ...
    for(int y = 0; y < this->height; y++) {
        for(int x = 0; x < this->width; x++) {
            int tile = furnitureData[y * width + x];
            if (tile < -1 || tile >= tile_count_x * tile_count_y) continue;
            if (tile == -1) continue;
...
bool Map::IsSolid(glm::vec3 position, float *penetration_x, float *penetration_y)
{
    ...
    *penetration_y = 0;
    ...
    if (tile < 8640 || tile > 16510) return false;
    int col = tile % tile_count_x;
    int row = tile / tile_count_x;
    if (col < 0 || col > 40 || row < 32 || row > 61) return false;
