...
void Effects::start(EffectType effect_type, glm::mat4* g_view_matrix)
{
    m_current_effect = effect_type;
    start(m_current_effect, 0.0f);
    gview_matrix = g_view_matrix;
}
...
void Effects::update(float delta_time)
{
   ...
   switch (m_current_effect)
   {
       ...
       case SHAKE: {
           ...
           view_matrix = glm::translate(glm::mat4(1.0f), glm::vec3(random_float, random_float, 0.0f));
//           gview_matrix = view_matrix * gview_matrix;
